...
function onMonsterHealthChanged(arg) {
    ...
    var pawn = arg.elementId ? [...pawns.all].find((x) => x.id == arg.elementId) : [...pawns.all].find((x) => x.index_in_main_window == index);
...
function startMovingMap(e) {
    ...
    function dragMoveMap(e) {
        if (disableMapDrag || e.target != gridLayer || (e.touches && e.touches.length > 1)) return;
        draggingMap = true;
        e = e || window.event;
...
function refreshMobBackgroundImages(pawn, bgArray) {
    ...
    if (mobsToAdd < 0) {
        ...
        if (bgArray) {
            ...
            bgArray.tokens.forEach((path) => {
                ...
                container.appendChild(createMobToken(base64, false, pawn));
            });
        } else {
            for (var i = 0; i < mobsToAdd; i++) {
                var path = tokenPaths.pickOne();
                container.appendChild(createMobToken(path, true, pawn));
            }
        }
    }
    ...
    for (var i = 0; i < shouldBeDead; i++) {
        ...
        var params = {
            classes: JSON.stringify(["mob_token_dead"]),
        };
        effectManager.makeElementIntoAnEffect(next, params);
    }
...
function createMobToken(path, cssify, mobElement) {
...
function stopMeasuring(event, ignoreClick) {
    ...
    if (ignoreClick || event.button == 2) {
        ...
        if (event != null) lastMeasuredPoint = { x: event.clientX, y: event.clientY };
    }
}
...
function setPawnBackgroundFromPathArray(newPawn, pawn) {
    ...
    imgEle.setAttribute("data-token_current_facet", pawn.tokenIndex);
    setPawnToken(newPawn, Util.cssify(selected));
}
            ...
            if (selectedPawns.length > 0) {
                selectedPawns.forEach((pwn) => {
                    ...
                    pwn.attached_objects.forEach((obj) => {
                        ...
                        if (obj.sound) soundManager.adjustPlacement(obj.id, obj.offsetLeft - posX, obj.offsetTop - posY);
                    });
                });
...
function addPawnListeners() {
    ...
    function allowAccess(pawn) {
        ...
        pawn.onwheel = function (event) {
            if (event.shiftKey) {
                if (event.deltaY > 0) {
                    pawnManager.rotatePawn(event.target, 3);
                } else {
                    pawnManager.rotatePawn(event.target, -3);
                }
...
async function setPawnMobBackgroundImages(pawn, pawnParams) {
    ...
    var possibleNames = await dataAccess.getTokenPaths(pawnParams.monsterId);
...
async function setPawnTokenFromParams(newPawn, pawn) {
    ...
    if (pawn.bgPhotoBase64) {
        return setPawnToken(newPawn, pawn.bgPhotoBase64);
    }

    if (pawn.bgPhoto) {
        return setPawnToken(newPawn, Util.cssify(pawn.bgPhoto));
    }
...
function setBasePawnProperties(newPawn, pawn) {
    newPawn.flying_height = 0;
    newPawn["data-dnd_conditions"] = [];
    if (!newPawn.dnd_hexes) {
...
async function createPawnElement(pawn, rotate) {
    var newPawn = document.createElement("div");
    ...
    pawnManager.rotatePawn(newPawn, rotate);

    if (pawn.scale) {
        pawnManager.setScale(newPawn, pawn.scale);
    }
    return newPawn;
}
...
function getPawnStartingRotation(pawn, isMonster) {
    ...
    if (isMonster) {
        ...
        return parseInt(settings.defaultPlayerTokenRotate);
    }
}
...
async function generatePawns(pawnArray, isMonster) {
    ...
    for (var i = 0; i < pawnArray.length; i++) {
        ...
        var newPawn = await createPawnElement(pawn, getPawnStartingRotation(pawn, isMonster));

        if (pawn.index_in_main_window) {
            var index = pawn.index_in_main_window;
            removeDuplicatePawnNumbers(pawn.index_in_main_window, newPawn.id);
...
const map = (function () {
    ...
    function updateObjectSize(ele) {
        var width, height;

        width = parseFloat(ele.dnd_width || 1);
        height = parseFloat(ele.dnd_height || 1);
        ele.style.width = normalizeSize(width) + "px";
        ele.style.height = normalizeSize(height) + "px";
    }

    function normalizeSize(baseSize) {
        return (baseSize * cellSize) / UNITS_PER_GRID;
    }

    function centerObjectOn(obj, pointX, pointY) {
        // calculate the new cursor position:
        width = normalizeSize(parseFloat(obj.dnd_width || 1));
        height = normalizeSize(parseFloat(obj.dnd_height || 1));
        var x = pointX - width / 2;
