...
var tokenFilePath;
var encounterModule = new EncounterModule();
...
$(document).ready(function () {
  ...
  $(".db_name_field").on("input", function (e) {
    ...
    if (tab == "monsters" || tab == "homebrew") document.getElementById("addTokenButton").disabled = nothingPresent;
  });
  ...
  document.getElementById("condition_image_picker").onclick = function (e) {
    conditionImagePath = dialog.showOpenDialog(remote.getCurrentWindow(), {
      properties: ['openFile'],
    ...
    if (conditionImagePath == null) return;
  ...
  document.getElementById("userCurrentPartyForEncounterDiff").onclick = function (e) {
    ...
    if (altInpCont.classList.contains("hidden")) {
      ...
      var levels = [];
      partyArray.filter(function (x) {
        return x.level && x.active;
      }).forEach(function (member) {
        return levels.push(parseInt(member.level));
      });
  ...
  dataAccess.getParty(function (data) {
    partyArray = data.members.filter(function (x) {
      return x.active;
    });
    ...
    if (partyArray) {
      ...
      if (partyArray.length > 0) {
        ...
        partyArray.forEach(function (member) {
          return charList += member.character_name + "(" + member.level + ")" + " ";
        });
      ...
      if (isNaN(highestLevel) || highestLevel > 30) highestLevel = 30; //Slider for encounter table
...
function filterDataListExecute() {
  ...
  if (tab == "monsters" || tab == "homebrew") {
    ...
    listedData = loadedData.filter(function (x) {
      return notNullAndContains(x.name, searchstring);
    });
  }
  ...
  function filterSpells() {
    ...
    if (classArray.length > 0) listedData = listedData.filter(function (x) {
      return x.classes != null && x.classes.filter(function (y) {
...
function notNullAndExactMatch(string, searchstring) {
  ...
  return string.toLowerCase() == searchstring || splt.filter(function (x) {
    return string == x;
  }).length > 0;
}
...
function populateSpellClassDropdown() {
  dataAccess.getSpells(function (data) {
    var worker = runWithWorker(function () {
      self.addEventListener("message", function (e) {
        ...
        data.forEach(function (element) {
          ...
          for (var j = 0; j < element.classes.length; j++) {
            ...
            if (!exists) {
              dropDownValues.push({
                value: element.classes[j],
                name: element.classes[j].substring(0, 1).toUpperCase() + element.classes[j].substring(1)
              });
            }
          }
        });
    ...
    worker.onmessage = function (e) {
      ...
      _toConsumableArray(document.getElementsByClassName("jsonValueClasses")).forEach(function (el) {
        new Awesomplete(el, {
          list: classList,
          autoFirst: true,
          minChars: 0
...
function setTab(x) {
  ...
  _toConsumableArray(searchBar).forEach(function (bar) {
    return bar.value = "";
  });
}
/**
  ...
  if (monsterNames == null) {
    dataAccess.getAutofillData(function (data) {
      data = data.filter(function (value) {
        return value[0].substring(0, 10) != "Encounter:";
      });
      monsterNames = data;
      fields = document.querySelectorAll(".specialjsonAttributeE");
      console.log(fields);
      fields.forEach(function (i) {
        awesomePletes.push(new Awesomplete(i, {
          list: monsterNames,
          autoFirst: true
        }));
...
function updateMonsterLists() {
  dataAccess.getMonsters(function (data) {
    dataAccess.getHomebrewMonsters(function (homebrewData) {
      data = data.concat(homebrewData);
      monsterMasterList = data;
      displayAddEncounterMonsterList();
      var types = [];
      monsterMasterList.forEach(function (mon) {
...
function displayAddEncounterMonsterList() {
  ...
  var filtered,
      filterTextSplt,
      filterText = document.getElementById("encounter_monster_list_search").value;

  if (filterText.replace(" ", "") != "") {
    filterTextSplt = filterText.split("&");

    if (filterTextSplt.length == 1) {
      filtered = monsterMasterList.filter(function (entry) {
        return notNullAndContains(entry.name, filterText) || notNullAndContains(entry.type, filterText) || notNullAndExactMatch(entry.challenge_rating + "", filterText);
      });
    } else {
      filtered = monsterMasterList.filter(function (entry) {
        var ret = true;
  ...
  console.log(filtered, monsterMasterList.filter(function (x) {
    return x.name.includes("Duergar");
  }));
  filtered.forEach(function (entry) {
    if (!entry.challenge_rating) entry.challenge_rating = 0;
    tr = document.createElement("tr");
    ["name", "type", "challenge_rating"].forEach(function (attribute) {
      var inner = "";
  ...
  $(".encounter_table>tbody>tr").on("click", function (e) {
    if ($(this).hasClass("selected_row")) {
      ...
      _toConsumableArray(document.querySelectorAll(".selected_row")).forEach(function (e) {
        return e.classList.remove("selected_row");
      });
    ...
    console.log(string);
    var foundMonster = monsterMasterList.filter(function (x) {
...
function addMonsterFromListToEncounter(e) {
  ...
  var freeNameInputs = _toConsumableArray(document.querySelectorAll(".specialjsonAttributeE")).filter(function (x) {
    return x.value == "" || x.value == monName;
  });

  if (freeNameInputs.length == 0) {
    addRow();
    freeNameInputs = _toConsumableArray(document.querySelectorAll(".specialjsonAttributeE")).filter(function (x) {
      return x.value == "";
    });
  }
...
function splitAndCheckSpellAttributes(searchString, attributes, motherList) {
  ...
  return motherList.filter(function (entry) {
    return filterTextSplt.filter(function (searchValue) {
      if (searchValue == "ritual" && entry.ritual) return true;
      var ret = false;
      attributes.forEach(function (attribute) {
        if (notNullAndContains(entry[attribute], searchValue)) ret = true;
      });
      return ret;
...
function sortEncounterMasterList(event) {
  ...
  if (sortAttribute == "challenge_rating") {
    encounterMonsterListSortFunction = function encounterMonsterListSortFunction(a, b) {
...
function getRandomEncounter() {
  ...
  nameEles.forEach(function (name) {
    return allowedMonsters.push(name.innerHTML);
  });
  var levels = [];
  partyArray.filter(function (x) {
    return x.level && x.active;
  ...
  encounterModule.getRandomEncounter(levels, pickOne(["easy", "medium", "hard", "deadly", "2x deadly"]), pickOne(["solitary", "squad", "mob"]), allowedMonsters, null, function (enc) {
    editObject(enc, "E");
  });
}
...
function deleteFromHomebrew(toRemove) {
  readDataFunction(function (data) {
    ...
    writeDataFunction(data, function () {
      window.setTimeout(function () {
        return loadAll();
      }, 200);
    });
...
function calculateEncounterDifficulty() {
  ...
  if (!altInpCont.classList.contains("hidden")) {
    ...
    for (var i = 0; i < partyArray.length; i++) {
      allLevels.push(partyArray[i].level);
    }
  }
  ...
  ["easy", "medium", "hard", "deadly"].forEach(function (difficulty) {
...
function loadAll() {
  readDataFunction(function (data) {
    loadedData = data; //skítamix þartil ég bæti við id á öðru
...
function listAll() {
  ...
  var allRows = _toConsumableArray($("#listFrame__" + tabElementName + ">.listRow"));
  ...
  if (tabElementName == "monsters") {
    ...
    for (var i = 0; i < data.length; i++) {
      ...
      if (data[i].source == "official") $("#listFrame__" + tabElementName + ">.listRow:nth-child(" + (i + 5) + ")").addClass("official_content_row");
      allRows[i + 1].setAttribute("data-entry_id", data[i].id);
...
function fillCurrentMonsterTokens(entryId) {
  document.querySelectorAll(".token").forEach(function (tok) {
    return tok.parentNode.removeChild(tok);
  });
  var i = 0;
...
function clearAddForm() {
  var element = document.getElementById("add" + tabElementNameSuffix); //Brute force this

  _toConsumableArray(element.querySelectorAll("input, textarea")).forEach(function (input) {
    if (!input.readOnly) input.value = "";
  });

  currentEntry = null;
  var letter = getLetterFromTabName();
  if (letter == "") document.querySelectorAll(".token").forEach(function (tok) {
    return tok.parentNode.removeChild(tok);
  });
}

function getLetterFromTabName() {
  if (tab == "monsters" || tab == "homebrew") return "";
...
function editCopyHelper(entryId, isEdit) {
  ...
  if (entryId) {
    entry = loadedData.find(function (x) {
      return x.id == entryId;
    });
  }
...
var hiddenAttributes = ["source", "id"];
...
function editObject(dataObject, letter) {
  ...
  var valuesElements = _toConsumableArray(document.querySelectorAll(".jsonValue" + letter));

  var keysElements = _toConsumableArray(document.querySelectorAll(".jsonAttribute" + letter));

  keysElements.forEach(function (x) {
    return x.value = "";
  });
  ...
  hiddenAttributes.forEach(function (attr) {
    if (dataObject[attr]) delete dataObject[attr];
  });
  if (letter === "S") fillClasses(dataObject);
  ...
  if (letter === "") fillNpcRequiredStats(loadedKeys, loadedValues); //Refetch

  if (addFieldsIfNeeded(loadedKeys.length - valuesElements.length)) {
    valuesElements = _toConsumableArray(document.querySelectorAll(".jsonValue" + letter));
    keysElements = _toConsumableArray(document.querySelectorAll(".jsonAttribute" + letter));
  }

  for (var i = 0; i < loadedKeys.length; i++) {
    if (["number", "string"].indexOf(_typeof(loadedValues[i])) != -1) {
      ...
      if (loadedKeys[i] == "actions" || loadedKeys[i] == "legendary_actions") {
        ...
        var difference = loadedValues[i].length - actionRows.length; //Tékka hvort það séu nógu margir fields og bæta við ef þarf. 
        ...
        subvalues.forEach(function (x) {
          x.value = "";
        });
        subkeys.forEach(function (x) {
          x.value = "";
        });
  ...
  if (tab == "encounters") {
    ...
    _toConsumableArray(fields).forEach(function (element) {
      return showCRForCreature(element);
    });
    ...
    if (dataObject["condition_color_value"]) document.getElementById("condition_color_picker").value = dataObject["condition_color_value"];
  }
  ...
  function fillNpcRequiredStats(keyArray, valueArray) {
    ...
    actionFields.forEach(function (x) {
      x.value = "";
    });
    specialFields.forEach(function (x) {
      x.value = "";
    });
    specialAttrFields.forEach(function (x) {
      x.value = "";
    });
    legActionFields.forEach(function (x) {
      x.value = "";
    });
  }
}
...
function populateTable(tableArray) {
  ...
  var elementArray = Object.values(tableArray); //Dálkar í DOMI eru færri en dálkar í json fylki
...
function addLookupHandlers() {
  ...
  $(".listRow:not(:nth-child(4))").children(".listRow__inner").on("click", function (e) {
    if ($(this).parent().hasClass("selected_row")) {
      hideStatblock();
      document.querySelectorAll(".selected_row").forEach(function (e) {
        return e.classList.remove("selected_row");
      });
      return;
    }
...
function addRow(str) {
  ...
  if (tab == "encounters") {
    ...
    [newCreatureInput, newNumberOfCreatures, newCreatureCr].forEach(function (element) {
...
function saveHomebrew() {
  ...
  for (var i = 0; i < valueBoxes.length; i++) {
    if (attributeBoxes[i].value != "" && valueBoxes[i].value != "" && valueBoxes[i].value != " " || tab != "monsters" && tab != "homebrew") {
      attribute = attributeBoxes[i].value.serialize();
  ...
  if (tab == "monsters" || tab == "homebrew") {
    ...
    var attribute = "special_abilities"; //populate actions

    addAttributeArray(document.getElementsByClassName("actionjsonValue"), document.getElementsByClassName("actionjsonAttribute"), "actions", thingyToSave); //populate legendary actions
  ...
  if (tab == "monsters" || tab == "encounters" || tab == "homebrew") {
    ...
    thingyToSave[attribute] = specialActions;
      ...
      var tableObject = {}; //Fylla út í töflu
      ...
      getFunction(function (data) {
        if (currentEntry) {
          data = data.filter(function (d) {
            return d.name != currentEntry.name;
          });
        }
        ...
        setFunction(data, function (err) {
          ...
          if (tokenRemoveQueue.length == 0) {
            ...
            while (tokenRemoveQueue.length > 0) {
              fs.unlink(tokenRemoveQueue.pop(), function (err) {
                if (err) throw err;
                if (tokenRemoveQueue.length == 0) saveTokens(thingyToSave.id);
              });
            }
          }
  ...
  function saveTokens(newId) {
    //Move tokens
    if (tab == "monsters" || tab == "homebrew") {
      var tokens = _toConsumableArray(document.querySelectorAll(".token"));
  ...
  function addProperty(property, object, fallbackValue) {
    var valueElement = document.getElementsByClassName("addmonster_" + property)[0];
    var keyElement = valueElement.parentNode.getElementsByClassName("attr")[0]; //Awesomplete haxxx
