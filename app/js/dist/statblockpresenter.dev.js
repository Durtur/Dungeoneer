...
var statblockPresenter = function () {
  ...
  var values;
  var statblock;
  var attributeNamesToIgnore = ["condition_color_value", "condition_background_location", "source", "id"];
  var attributeNamesToHeader = ["name"];
  var attributesWithoutNames = [" ", "description"];
  ...
  function createStatblock(_statblock, valueElement, statblockType) {
    ...
    [addClasses, addName, addRitual, addSpellProperties, storeDescription, createTypeDescription, addHpAndAc, addAbilityRow, morphActions, storeTable, storeHigherLevels, populateRemainingStats, addDescription, addTable, createAndAddEncounterDescription, addHigherLevels, finalizeStatblock].forEach(function (step) {
  ...
  function addDescription() {
    if (statblockDescriptionText == null) return;
  ...
  function addHpAndAc() {
    if (!values.hit_points && !values.armor_class) return;
    var hpAndACRow = document.createElement("div");
    hpAndACRow.classList = "statblock_hp_and_ac_row";
    if (values.hit_points) createCol("hit_points");
    if (values.armor_class) createCol("armor_class");
    if (values.challenge_rating) createCol("challenge_rating", "CR", (encounterModule ? encounterModule : new EncounterModule()).getXpValueForCR(values.challenge_rating) + " xp");
  ...
  function addAbilityRow() {
    ...
    monsterAbilityValues.forEach(function (ability) {
  ...
  function addSpellProperties() {
    ...
    if (values["level"]) if (values.level == 0 || values.level == "0") values.level = "Cantrip";
    var props = ["level", "school", "range", "casting_time", "duration", "components"];
    var col,
        hasAnySpellProp = false;
    ...
    if (hasAnySpellProp) statblock.appendChild(divCont);

    function processProp() {}
  }

  function createAndAddEncounterDescription() {
    if (values.encounter_xp_value && partyArray.filter(function (p) {
      return p.active;
    }).length > 0) {
      var createEncounterDifficultyElement = function createEncounterDifficultyElement(description, totalXp, partyDescString) {
        var newP = document.createElement("p");
      ...
      var allLevels = [];
      var levelCounts = [];
      partyArray.filter(function (p) {
        return p.active;
      }).forEach(function (partyMember) {
      ...
      var levelsLeft = 0;
      levelCounts.forEach(function (x) {
        if (x) levelsLeft++;
      });
  ...
  function createTypeDescription() {
    var str = values.size != null ? values.size : values.rarity != null ? values.rarity : "";
    delete values.size;
  ...
  function populateStats(k, v, statblock) {
    if (Array.isArray(v)) {
      if (v.length > 0) statblock.appendChild(createHeader(k));
      ...
      if (typeof k == "string" && k.toLowerCase().indexOf("spellcast") >= 0 | typeof v == "string" && v.toLowerCase().indexOf("spell") >= 0) spellCastingRootNodes.push(k);
  ...
  function finalizeStatblock() {
    ...
    if (spellCastingRootNodes.length > 0) spellcastingLinkController.updateLinks(statblock);
    diceRollerLinkController.updateLinks(statblock);
  }

  var diceRollerLinkController = function () {
    function updateLinks(statblock) {
      var paragraphs = _toConsumableArray(statblock.getElementsByTagName("p"));
      ...
      window.setTimeout(function () {
        _toConsumableArray(document.getElementsByClassName("d20_roll_link")).forEach(function (d20Handler) {
        ...
        _toConsumableArray(document.getElementsByClassName("dice_roller_link")).forEach(function (d20Handler) {
          d20Handler.addEventListener("click", function (e) {
  ...
  var spellcastingLinkController = function () {
    ...
    function updateLinks(statblock) {
      ...
      spellCastingRootNodes.forEach(function (node) {
        var currAttribute = foundMonster[node];
        spellcastingAttribute += " ";

        if (!currAttribute) {
          if (foundMonster.special_abilities) foundMonster.special_abilities.forEach(function (ability) {
            if (ability[node]) {
      ...
      if (spellcastingAttribute == "") return;
    ...
    function createHandlersForSpellLinks() {
      var allLinks = _toConsumableArray(document.getElementsByClassName("spell_link"));

      allLinks.forEach(function (link) {
        return link.addEventListener("mouseenter", function (e) {
          e.target.setAttribute("data-mouse-over", "t");
          clearTimeout(delayedOpening);
          delayedOpening = window.setTimeout(function () {
            if (document.getElementById("spell_popup").getAttribute("data-mouse-over") == "t" || e.target.getAttribute("data-mouse-over") == "f") return;
      ...
      allLinks.forEach(function (link) {
        return link.addEventListener("mouseleave", function (e) {
          e.target.setAttribute("data-mouse-over", "f");
    ...
    function updateLinksForParagraph(paragraph) {
      ...
      knownSpells.forEach(function (spell) {
        ...
        if (spellIndex >= 0) {
          ...
          innerText = innerText.substring(0, spellIndex) + newAnchor.outerHTML + innerText.substring(spellIndex + spellNameLength, innerText.length);
      ...
      if (paragraph.nextElementSibling && paragraph.nextElementSibling.getAttribute("data-spell-links-updated") != "t") {
