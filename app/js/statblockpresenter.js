...
class StatblockPresenter {
    constructor(_statblock, valueElement, _statblockType, editable) {
        ...
        if (editable) this.editMode = true;
    ...
    async createStatblock(statblock, valueElement, statblockType, editMode) {
        var cls = this;
        while (statblock.firstChild) statblock.removeChild(statblock.firstChild);
        ...
        async function addName() {
            ...
            var tokenEle = elementCreator.createTokenElement(path).token;
            tokenEle.onclick = async () => {
                await cls.tokenSelector.getNewTokenPaths(
                    false,
                    (paths) => {
                        if (!paths) return;
                        var path = paths[0];
        ...
        function addDescription() {
            if (!statblockDescriptionText) return;
        ...
        function createHeaderSmaller(text) {
            var createEditButton = editMode && statblockEntry.actions?.find((x) => x.name == text) && constants.weapons.find((x) => x.name.toLowerCase() == text.toLowerCase());
        ...
        function createEditAcButton() {
            ...
            input.addEventListener("awesomplete-selectcomplete", (evt) => {
                ...
                if (selectedArmor.is_shield) {
                    if (statblockEntry.ac_source.find((x) => x.toLowerCase() == selectedArmor.type.toLowerCase())) return;
                }
        ...
        function populateStats(k, v, statblock) {
            if (Array.isArray(v)) {
                if (v.length > 0 && v.constructor != Object) statblock.appendChild(createHeader(k));
                ...
                if (typeof k == "string" && typeof v == "string" && (k.toLowerCase().indexOf("spellcast") >= 0 || v.toLowerCase().indexOf("spell") >= 0)) spellCastingRootNodes.push(k);
        ...
        function finalizeStatblock() {
            ...
            if (statblockType == "spells") spellCastingRootNodes.push("description");
            if (spellCastingRootNodes.length > 0) spellcastingLinkController.updateLinks(statblock, statblockEntry, spellCastingRootNodes);
