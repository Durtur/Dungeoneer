...
document.addEventListener("DOMContentLoaded", function () {
    ...
    document.getElementById("lootcr").oninput = function () {
        randomizeLoot();
    };
    document.getElementById("lootcr").onkeydown = function (evt) {
        if (evt.key == "Enter") randomizeLoot();
    };
...
function addPcNodeCondition(node, condition, originMapTool = false) {
    ...
    if (condList.includes(condition)) return;
    condList.push(condition);

    if (!originMapTool) notifyMapToolConditionsChanged(node.querySelector(".pcnode_name>p").innerHTML, condList, true);
...
function hideAllPopups(evt) {
    if (evt && evt.target.closest(".popup_menu")) return;
...
function search(key, showStatblock, optionalSearchString, ignoreSearchInput) {
    ...
    switch (key) {
        case "monsters":
            getFunction = dataAccess.getHomebrewAndMonsters;
            break;
        case "encounters":
...
function filterPcRowsBySelectedParty(isInitialLoad) {
    ...
    var emphasisColor = getComputedStyle(document.documentElement).getPropertyValue("--alt-emphasis-color");
