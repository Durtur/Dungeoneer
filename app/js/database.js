...
function populateDropdowns() {
    ...
    document.querySelector(".addmonster_dexterity").oninput = (evt) => {
        armorSelected();
    };
...
function populateAddableFieldDropdowns() {
    ...
    function specialAbilitySelected(evt) {
        ...
        var desc = selectedAbility.description.replace(/@{CREATURE_NAME}/g, function (match, offset, string) {
            if (offset == 0 || string[offset] == "." || string[Math.max(offset - 1, 0)] == ".") return creatureName.toProperCase();
            return creatureName;
        });
...
function filterDataListExecute() {
    ...
    if (tab == "monsters" || tab == "homebrew") {
        ...
        listedData = loadedData.filter((x) => {
    ...
    function filterSpells() {
        ...
        if (classArray.length > 0)
            listedData = listedData.filter((x) => {
                return (
                    x.classes != null &&
                    x.classes.filter((y) => {
...
function notNullAndExactMatch(string, searchstring) {
    ...
    return string.toLowerCase() == searchstring || splt.filter((x) => string == x).length > 0;
}
...
function addClassTextBoxForSpells() {
    ...
    new Awesomplete(newInput, { list: classList, autoFirst: true, minChars: 0 });
    newInput.focus();
}

var readDataFunction, writeDataFunction;
function setTab(tabName) {
    $(".mainheader").html(tabName);
    localStorage.setItem("database-selected-tab", tabName);
    hide("add" + tabElementNameSuffix);
    listedData = null;
    closeListFrame();
    fileName = tabName + ".json";
    tab = tabName;
    ...
    document.getElementById(tabName).classList.add("toggle_button_toggled");
...
function resizeTable(horizontal, add) {
    ...
    if (horizontal != 1) {
        ...
        if (add == 1) {
            $(".jsonTableRow" + letter + ":nth-child(2)")
                .clone()
                .appendTo($(".jsonTable" + letter));
        } else {
...
function splitAndCheckSpellAttributes(searchString, attributes, motherList) {
    ...
    return motherList.filter((entry) => {
        return (
            filterTextSplt.filter((searchValue) => {
...
function getRandomEncounter() {
    ...
    encounterModule.getRandomEncounter(levels, ["easy", "medium", "hard", "deadly", "2x deadly"].pickOne(), ["solitary", "squad", "mob"].pickOne(), allowedMonsters, null, null, function (enc) {
...
async function fillCurrentMonsterTokens(entryId, isCopy) {
    document.querySelectorAll(".token").forEach((tok) => tok.parentNode.removeChild(tok));

    var paths = await dataAccess.getTokenPaths(entryId);
...
function createToken(pathStr, isOldToken, isCopy) {
    ...
    token.setAttribute("src", pathStr);
...
async function editEntry(entryId) {
    await editCopyHelper(entryId, true);
}
...
async function editCopyHelper(entryId, isEdit) {
    clearAddForm();
    var entry;
    if (entryId) {
    ...
    if (letter == "") {
        ...
        await fillCurrentMonsterTokens(entryId, !isEdit);
    }
...
function editSpell(dataObject) {
    ...
    ["name", "level", "description", "casting_time", "duration", "higher_levels", "range", "components", "school"].forEach((prop) => {
...
function editObject(dataObject, letter) {
    ...
    function editItem(dataObject) {
        ...
        if (dataObject.table) populateTable(dataObject.table);
    }
...
function addRow(str) {
    ...
    if (str != null) {
        if (str == "special_abilities" || str == "actions") {
            while (specialAbilityAndActionAwesompletes.length > 0) specialAbilityAndActionAwesompletes.pop().destroy();
            $(".addRow_" + str + ">.row:first-child")
                .first()
                .clone()
                .appendTo($(".addRow_" + str));
    ...
    if (tab == "encounters") {
        ...
        var newRow = $(container + ">.row>.jsonAttribute")
            .first()
            .parent()
            .clone();
...
function saveHomebrew() {
    ...
    readDataFunction((data) => {
        ...
        async function saveDataObject(thingyToSave) {
            console.log("CurrentlyEditing ", currentEntry);
            if (!thingyToSave.id) thingyToSave.id = uniqueID();
            if (tab == "monsters" && !currentEntry) {
                return await handleDataSave(thingyToSave, dataAccess.getHomebrewMonsters, dataAccess.setHomebrewMonsters);
            } else {
                return await handleDataSave(thingyToSave, readDataFunction, writeDataFunction);
            }

            async function handleDataSave(thingyToSave, getFunction, setFunction) {
                getFunction(async function (data) {
                    if (currentEntry) {
                    ...
                    setFunction(data, async function (err) {
                        // hide('add' + tabElementNameSuffix);
                        ...
                        if (tab == "monsters" || tab == "homebrew") {
                            await saveTokens(thingyToSave.id);
                            tokenRemoveQueue = [];
        ...
        async function saveTokens(newId) {
            //Move tokens
            if (tab != "monsters" && tab != "homebrew") return;
            var tokens = [...document.querySelectorAll(".token")];
            var newTokens = tokens.filter((x) => x.getAttribute("data-is_new_token"));

            await dataAccess.commitTokenSave(
                newId,
        ...
        function AddReactions(thingyToSave) {
            ...
            if (reactionArr.length > 0) thingyToSave.reactions = reactionArr;
        }
    });
}
