...
window.subscribe.on("maptool-server-event", async (event, message) => {
  ...
  if (
    ["foreground", "background", "overlay", "map_edge"].find(
      (x) => x == message.event
    )
  ) {
...
function getDefaultAccess(peer) {
  ...
  var access = [];
  var players = partyArray.filter(
    (x) => x.player_name.toLowerCase() == peer.name.toLowerCase()
  );
  if (players.length > 0) {
    access = players;
  }
...
function createPeerElement(peer) {
  ...
  ele.appendChild(p);
  [{ character_name: "Full control", id: "all" }, ...partyArray].forEach(
    (player) => {
      var isChecked = peer.partyAccess.find((x) => x.id == player.id) != null;

      var checkBox = ElementCreator.checkBox(
        ...
        (e) => {
          ...
          if (!checked) {
            peer.partyAccess = peer.partyAccess.filter(
              (x) => x.id != player.id
            );
          } else {
...
function appendServerLog(text, eventType) {
  ...
  if (eventType == SERVER_EVENTS.CONNECTED) {
    wrapLog(
      util.ele("p", "server_activity_log_entry log_entry_connected", text)
    );
  ...
  function wrapLog(entry) {
    var timestamp = util.ele(
      "p",
      "server_activity_log_timestamp ",
...
async function getConnectionParams() {
  ...
  try {
    const response = await fetch(
      "https://raw.githubusercontent.com/Durtur/Dungeoneer/master/docs/client/connectionConfig.js",
      { cache: "no-cache" }
    );
    if (response.ok) {
...
async function initServer() {
  ...
  peer = hostId
    ? new Peer(hostId, CONNECTION_PARAMS)
    : new Peer(CONNECTION_PARAMS);
  peer.on("open", function (id) {
    document.getElementById("server_id").value = id;
    localStorage.setItem("server_id", id);
    [...document.querySelectorAll(".server_active_container")].forEach((x) =>
      x.classList.remove("hidden")
    );
    [...document.querySelectorAll(".server_inactive_container")].forEach((x) =>
...
function onConnected(conn) {
  ...
  if (pass) {
    if (conn.metadata.password != pass) {
      conn.close();
      appendServerLog(
        `Rejected ${conn.connectionId} due to incorrect password`,
        SERVER_EVENTS.ERROR
      );
      return;
    }
...
function handleDataEvent(data, connection) {
  ...
  if (data.event == "init") {
    ...
    data.data.forEach((ele) => {
      var access = peer.partyAccess.find(
        (x) => x.element_id == ele.id || x.element_id == "all"
      );
      ...
      if (access) {
        ...
        if (player || ele.idx) {
          ...
          appendServerLog(
            `${peer.name} moved ${pwnName} ${ele.distance}`,
            SERVER_EVENTS.MOVE
          );
        }

        var otherPeers = peers.filter(
          (x) => x.connection.connectionId != peer.connection.connectionId
        );
...
async function getMapLayer(path, cacheKey) {
  ...
  if (
    ...
    EXPORT_LAYER_CACHE[cacheKey].path == path
  ) {
...
function sendMaptoolState(maptoolState) {
  dataAccess.getSettings(async (settings) => {
    while (pendingStateRequests.length > 0) {
      ...
      var dataObject = {
        foreground: await getMapLayer(
          maptoolState.foreground.path,
          "foreground"
        ),
        background: await getMapLayer(
          maptoolState.background.path,
          "background"
        ),
