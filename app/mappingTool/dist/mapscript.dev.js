...
var Awesomplete = require(path.resolve('app/awesomplete/awesomplete.js'));
...
var zIndexPawnCap = 9;
var conditionList; //Stillibreytur
...
var pawns = function () {
  ...
  var lastLocationPlayers = {
    x: 3,
    y: 3
  };
  var lastLocationMonsters = {
    x: 18,
    y: 3
  };
  var addQueue = [];
...
function loadSettings() {
  dataAccess.getSettings(function (data) {
    ...
    if (settings.map_edge_style) document.querySelector(".maptool_body").style.backgroundImage = "url('" + settings.map_edge_style + "')";
...
function showVisibilityLayer() {
  ...
  if (visibilityLayerEnabled == "false") {
    ...
    var visToolbox = _toConsumableArray(document.getElementsByClassName("visibility_toolbox"));

    visToolbox.forEach(function (ele) {
      return ele.classList.remove("hidden");
    }); //Hide light effects that are in the normal layer

    var effectsToHide = _toConsumableArray(document.getElementsByClassName("light_source_normal_layer"));

    effectsToHide.forEach(function (effect) {
      return effect.classList.add("hidden");
    });

    var effectsToShow = _toConsumableArray(document.getElementsByClassName("light_source_visibility_layer"));

    effectsToShow.forEach(function (effect) {
      return effect.classList.remove("hidden");
    });
    if (fogOfWarEnabled) document.getElementById("fov_tool").click();
  } else {
    ...
    var effectsToHide = _toConsumableArray(document.getElementsByClassName("light_source_visibility_layer"));

    effectsToHide.forEach(function (effect) {
      return effect.classList.add("hidden");
    });

    var effectsToShow = _toConsumableArray(document.getElementsByClassName("light_source_normal_layer"));
...
ipcRenderer.on("notify-main-reloaded", function () {
  pawns.players.forEach(function (pawn) {
    return raiseConditionsChanged(pawn[0]);
  });
});
ipcRenderer.on('token-condition-added', function (evt, list, index) {
  var pawn = _toConsumableArray(document.querySelectorAll(".pawn_numbered")).filter(function (pw) {
    return pw.index_in_main_window == index;
  })[0];

  if (pawn) {
    removeAllPawnConditions(pawn);
    list.forEach(function (cond) {
      return setPawnCondition(pawn, conditionList.filter(function (x) {
        return x.name.toLowerCase() == cond;
      })[0]);
    });
  }
});
ipcRenderer.on('monster-health-changed', function (evt, arg) {
  ...
  constants.creatureWounds.forEach(function (woundType) {
    return woundEle.classList.remove(woundType.className);
  });
  var woundType = constants.creatureWounds.find(function (x) {
    return arg.healthPercentage < x.percentage;
  });
...
ipcRenderer.on('notify-map-tool-mob-changed', function (evt, arg) {
  ...
  list.forEach(function (param) {
    var pawn = loadedMonstersFromMain.find(function (x) {
      return x.index_in_main_window == param.rowIndex;
    });
    if (!pawn) return;
...
ipcRenderer.on('monster-list-cleared', function (evt, arg) {
  ...
  loadedMonstersFromMain.forEach(function (element) {
    if (element.getAttribute("data-mob_size") != null) return;
...
ipcRenderer.on("notify-map-tool-monsters-loaded", function (evt, arg) {
  ...
  console.log(monsterArray); //Analísera til að dreifa litum
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  loadSettings();
  var window2 = remote.getGlobal('mainWindow');
  if (window2) window2.webContents.send('maptool-initialized');
  var bgSize = parseInt($("#background").css("background-size"));
  var slider = document.getElementById("map_size_slider") < document.addEventListener("visibilitychange", function () {
    if (document.hidden) {
  ...
  function pawnBgColorChosen(color) {
    newColor = color;
    selectedPawns.forEach(function (element) {
  ...
  dataAccess.getConditions(function (data) {
    ...
    var list = conditionList.map(function (x) {
      return x.name;
    }).sort();
    ...
    input.addEventListener('awesomplete-selectcomplete', function (e) {
      ...
      var condition = conditionList.find(function (c) {
...
function moveMap(x, y) {
  [backgroundCanvas].forEach(function (layer) {
...
function onSettingsLoaded() {
  ...
  document.getElementById("add_sfx_dropdown").onclick = effectDropdownChange; //  document.getElementById("add_sfx_dropdown").onchange = selectedSfxChanged;
  ...
  document.querySelector("body").onkeydown = function (event) {
    ...
    if (event.keyCode == 37 || event.keyCode == 65) {
      bgX += canvasMoveRate;
      moveOffsetX += canvasMoveRate; //right
    } else if (event.keyCode == 39 || event.keyCode == 68) {
      bgX -= canvasMoveRate;
      moveOffsetX -= canvasMoveRate; //up
    } else if (event.keyCode == 38 || event.keyCode == 87) {
      bgY += canvasMoveRate;
      moveOffsetY += canvasMoveRate; //down
  ...
  var iconLoadButtons = _toConsumableArray(document.querySelectorAll(".icon_load_button"));

  iconLoadButtons.forEach(function (button) {
  ...
  document.getElementById("save_map_button").onclick = function (e) {
    ...
    if (path != null) {
      ...
      for (var i = 0; i < effects.length; i++) {
        data.effects[i].data_classList = _toConsumableArray(effects[i].classList);
  ...
  document.getElementById("load_map_button").onclick = function (e) {
    ...
    if (path != null) {
      fs.readFile(path, function (err, data) {
        ...
        data = JSON.parse(data); //  pawns = data.pawns;
        ...
        moveMap(data.bgX, data.bgY); //Light effects

        var oldEffects = _toConsumableArray(tokenLayer.getElementsByClassName("light_effect"));

        while (oldEffects.length > 0) {
          var oldEffect = oldEffects.pop();
          pawns.lightSources = pawns.lightSources.filter(function (item) {
            return item !== oldEffect;
          });
          tokenLayer.removeChild(oldEffect);
        } //Standard effects


        oldEffects = _toConsumableArray(tokenLayer.getElementsByClassName("sfx_effect"));
        ...
        if (data.pawns) {
          data.pawns.forEach(function (p) {
            var newP = document.createElement("div");
            if (p.lightEffect) pawns.lightEffects.push(newP);
...
function createEffectMenus() {
  dataAccess.getMapToolData(function (data) {
    effectData = data.effects;
    createMenu("add_sfx_dropdown", effectData.filter(function (x) {
      return !x.isLightEffect;
    }));
    createMenu("add_light_source_dropdown", effectData.filter(function (x) {
      return x.isLightEffect;
    }));
    var newOption = document.createElement("option");
...
function restoreEffect(effect) {
  ...
  effect.data_classList.forEach(function (className) {
    return newEffect.classList.add(className);
  });
...
function zoomIntoMap(event, resizeAmount) {
  window.requestAnimationFrame(function (ts) {
    ...
    cellSize = originalCellSize * newSize; //Move pawns
    ...
    [pawns.all, effects].forEach(function (arr) {
      for (var i = 0; i < arr.length; i++) {
        var pawn = arr[i];
        var left = parseFloat(pawn.style.left);
        var top = parseFloat(pawn.style.top);
        cellsFromLeft = (left - backgroundOriginX) / (originalCellSize * backgroundSizeBeforeResize);
        cellsFromTop = (top - backgroundOriginY) / (originalCellSize * backgroundSizeBeforeResize);
    ...
    fovLighting.resizeSegments({
      x: backgroundOriginX,
      y: backgroundOriginY
    }, {
      x: newBackgroundOriginX,
      y: newBackgroundOriginY
    }, backgroundSizeBeforeResize);
    window.requestAnimationFrame(refreshFogOfWar);
  });
}
...
var backgroundLoop = function () {
  ...
  function setBackgroundSlide(button) {
    ...
    if (!slideCanvas.style.backgroundPositionX) slideCanvas.style.backgroundPositionX = "0";
    if (!slideCanvas.style.backgroundPositionY) slideCanvas.style.backgroundPositionY = "0";
    var loop;
...
function startMeasuring(event) {
      ...
      if (event.button == 0) {
        if (event.ctrlKey) {
          if (lastMeasuredLineDrawn) {
            totalMeasuredDistance += Math.round(Math.sqrt(Math.pow(lastMeasuredLineDrawn.a.x - lastMeasuredLineDrawn.b.x, 2) + Math.pow(lastMeasuredLineDrawn.a.y - lastMeasuredLineDrawn.b.y, 2) + Math.pow(lastMeasuredLineDrawn.a.z - lastMeasuredLineDrawn.b.z, 2)) / cellSize * 5);
          }
  ...
  function measureSphereSegment(event) {
    ...
    window.requestAnimationFrame(function () {
      ...
      var radius = Math.sqrt(Math.pow(event.clientX - measurementOriginPosition.x, 2) + Math.pow(event.clientY - measurementOriginPosition.y, 2));
      measurementsLayerContext.beginPath();
      measurementsLayerContext.moveTo(measurementOriginPosition.x, measurementOriginPosition.y);
  ...
  function measureCone(event) {
    window.requestAnimationFrame(function () {
      ...
      measurementsLayerContext.beginPath();
      measurementsLayerContext.moveTo(measurementOriginPosition.x, measurementOriginPosition.y);
      var newPoint = rotate(0.46355945, measurementOriginPosition, {
        x: event.clientX,
        y: event.clientY
      });
      var newPoint2 = rotate(-0.46355944999997217, measurementOriginPosition, {
        x: event.clientX,
        y: event.clientY
      });
      var midPoint = {
      ...
      measurementsLayerContext.stroke();
      measurementsLayerContext.fill();
      showToolTip(event, Math.round(Math.sqrt(Math.pow(midPoint.x - measurementOriginPosition.x, 2) + Math.pow(midPoint.y - measurementOriginPosition.y, 2)) / cellSize * 5) + " ft", "tooltip");
      lastMeasuredCone.x = measurementOriginPosition.x;
      lastMeasuredCone.y = measurementOriginPosition.y;
      lastMeasuredCone.radius = Math.sqrt(Math.pow(event.clientX - measurementOriginPosition.x, 2) + Math.pow(event.clientY - measurementOriginPosition.y, 2));
    });
  }
  ...
  function rotate(angle, origin, coords) {
    ...
    return {
      "x": parseInt(x + origin.x * 1),
      "y": parseInt(y + origin.y * 1)
    };
  }
...
function refreshMeasurementTooltip() {
  ...
  if (measurementTargetDestination != null && measurementTargetOrigin != null) {
    ...
    tooltip.innerHTML = Math.round(Math.sqrt(Math.pow(destinationPoint.x - originPosition.x, 2) + Math.pow(destinationPoint.y - originPosition.y, 2) + Math.pow(destinationPoint.z - originPosition.z, 2)) / cellSize * 5) + " ft";
  }
}
...
function stopMeasuring(event, ignoreClick) {
  if (ignoreClick || event.button == 2) {
    ...
    if (fovToolbox[0]) {
      ...
      fovLighting.addLineSegment(lastMeasuredPoint, {
        x: event.clientX,
        y: event.clientY
      });
    } else if (fovToolbox[1]) {
      fovLighting.addRectangleSegment(lastMeasuredPoint, {
        x: event.clientX,
        y: event.clientY
      });
    } else if (fovToolbox[2]) {
      fovLighting.addSphereSegment(lastMeasuredPoint, {
        x: event.clientX,
        y: event.clientY
      });
    }
    ...
    if (event != null) lastMeasuredPoint = {
      x: event.clientX,
      y: event.clientY
    };
  }
...
function generatePawns(pawnArray, monsters, optionalSpawnPoint) {
  ...
  for (var i = 0; i < pawnArray.length; i++) {
    ...
    if (pawn.isMob) {
      ...
      newPawn.appendChild(newPawnStatus); //Checka hvort gefið hafi verið input fæll
      ...
      if (optionalPaths != null) {
        ...
        monsters ? setPawnImageWithDefaultPath(newPawn, pawn.monsterId) : setPlayerPawnImage(newPawn, pawn.id);
      }
    }
...
function removeDuplicatePawnNumbers(index) {
  var pawns = _toConsumableArray(document.getElementsByClassName("pawn_numbered"));
...
function addToPawnBackgrounds(element, paths) {
  ...
  paths.forEach(function (path) {
    ...
    if (currentIndex < 0) currentPaths.push(path);
  });
...
function refreshMobBackgroundImages(pawn) {
  ...
  var allTokens = _toConsumableArray(pawn.querySelectorAll(".mob_token"));

  if (allTokens.length == 0) return removePawn(pawn); //Make them dead  

  var alivePawns = allTokens.filter(function (x) {
    return !x.classList.contains("mob_token_dead");
  });
  ...
  if (_toConsumableArray(pawn.querySelectorAll(".mob_token")).length == 0) {
  ...
  var stepX = 5;
  var stepY = 5;
  alivePawns.forEach(function (ele) {
...
function removeAllConditionsHandler(event) {
  selectedPawns.forEach(function (pawn) {
    ...
    var conditions = _toConsumableArray(pawn.getElementsByClassName("condition_effect"));
...
function setPawnCondition(pawnElement, condition) {
  ...
  if (!conditionString || pawnElement["data-dnd_conditions"].indexOf(conditionString) > -1) return;
...
function removePawnConditionHelper(pawnElement, conditionObj, deleteAll) {
  if (deleteAll) {
    ...
    pawnElement["data-dnd_conditions"] = currentArr.filter(function (x) {
      return x != conditionObj.name;
    });
  }

  var allConditions = _toConsumableArray(pawnElement.getElementsByClassName("condition_effect"));
...
function raiseConditionsChanged(pawn) {
  ...
  var window2 = remote.getGlobal('mainWindow');
  if (window2) window2.webContents.send('condition-list-changed', pawn["data-dnd_conditions"], pawn.index_in_main_window ? pawn.index_in_main_window : pawn.title);
}
...
function enlargeReducePawn(direction) {
  selectedPawns.forEach(function (element) {
    return enlargeReduceHelper(element, direction);
  });
  refreshPawns();
  resizePawns();
...
function killOrRevivePawn() {
  ...
  function killOrReviveHelper(pawnElement) {
    ...
    if (revivePawn) {
      ...
      if (!isPlayer) {
        if (loadedMonstersFromMain.indexOf(pawnElement) >= 0) {
          var _window = remote.getGlobal('mainWindow');

          if (_window) _window.webContents.send('monster-killed', [pawnElement.dnd_name, pawnElement.index_in_main_window]);
        }
      }
    }
...
function startAddingFromQueue() {
  ...
  function popQueue(e) {
    ...
    var pawn = generatePawns([pawns.addQueue[0]], true, {
      x: e.clientX - offset,
      y: e.clientY - offset
    });
...
function setFillStyle() {
  measurementFillStylePath = dialog.showOpenDialog(remote.getCurrentWindow(), {
    properties: ['openFile'],
    message: "Choose picture location",
    filters: [{
      name: 'Images',
  ...
  if (measurementFillStylePath) measurementFillStylePath = measurementFillStylePath.replace(/\\/g, "/");
...
function addSfxEffectHandler(e, isPreviewElement) {
  ...
  var effectObj = effectData.filter(function (x) {
    return x.name == effectName;
  })[0];
...
function createBaseEffect(effectObj, isPreviewElement, e) {
  ...
  if (effectObj.classes) {
    effectObj.classes.forEach(function (effClass) {
      return newEffect.classList.add(effClass);
    });
  }
  ...
  newEffect.style.backgroundImage = selectedSfxBackground; //Refresh preview

  if (!isPreviewElement) previewPlacement(createEffect(e, true));
  effects.push(newEffect);
...
function setTokenImageHandler(e) {
  ...
  if (imagePaths != null) {
    if (e.target == input) {
      selectedPawns.forEach(function (element) {
        return setPawnBackgroundFromPathArray(element, imagePaths);
      });
    } else if (e.target == facetButton) {
      selectedPawns.forEach(function (element) {
        return addToPawnBackgrounds(element, imagePaths);
      });
    }
  }
}
...
function stopAddingEffects() {
  ...
  effects = effects.filter(function (eff) {
    return eff != previewPlacementElement;
  });
}
...
function createConditionButton(condition) {
  ...
  btn.onclick = function (e) {
    ...
    selectedPawns.forEach(function (pawn) {
      return removePawnCondition(pawn, conditionList.find(function (x) {
        return x.name == name;
      }));
    });
...
function showConditionsMenu(event) {
  ...
  var buttons = _toConsumableArray(menuWindow.getElementsByClassName("condition_button"));
  ...
  selectedPawns.forEach(function (pawn) {
    ...
    pawn["data-dnd_conditions"].forEach(function (condition) {
      if (conditionsAdded.find(function (x) {
        return x == condition;
      })) return;
...
function isSelectedPawn(pawn) {
  for (var i = 0; i < selectedPawns.length; i++) {
    if (selectedPawns[i] == pawn) return i;
  }
...
function dragPawn(elmnt) {
  ...
  function dragMouseDown(e) {
    if (elmnt.data_overload_click) return elmnt.data_overload_click(e); //Line tool
    ...
    if (toolbox[0]) {
      if (measurementTargetOrigin == null) {
        ...
        drawLineAndShowTooltip(measurementOriginPosition, {
          x: e.clientX,
          y: e.clientY,
    ...
    if (e.buttons == 1) {
      ...
      if (selectedPawns.length == 1) clearSelectedPawns();
    ...
    window.requestAnimationFrame(function (ts) {
      ...
      if (selectedPawns.length > 0) {
        for (var i = 0; i < selectedPawns.length; i++) {
          ...
          pwn.attached_objects.forEach(function (obj) {
        ...
        elmnt.attached_objects.forEach(function (obj) {
          obj.style.top = obj.offsetTop - pos2 + "px";
          obj.style.left = obj.offsetLeft - pos1 + "px";
        }); //Clear old
...
function resizeHelper(arr) {
  for (var i = 0; i < arr.length; i++) {
    ...
    if (mobSize) {
      ...
      var mobTokens = _toConsumableArray(pawn.querySelectorAll(".mob_token"));

      mobTokens.forEach(function (token) {
...
function resizeAndDrawGrid(timestamp, event) {
  if (timestamp) {
    if (gridResize_Timestamp == timestamp) return;
  ...
  drawGrid();
  if (previewPlacementElement) adjustPreviewPlacement(event);
}
