...
function loadSettings() {
    dataAccess.getSettings(function (data) {
        ...
        if (settings.currentMap) {
            setMapBackground(settings.currentMap);
        }
...
ipcRenderer.on('monster-killed', function (evt, arg) {
    ...
    console.log(killedPawn.dnd_name + " was killed");
...
ipcRenderer.on("next-player-round", function (evt, params) {
    console.log("Next player roud", params)
...
function onSettingsLoaded() {
    ...
    document.querySelector("#map_edge_button").onclick = function (e) {
        ...
        console.log(document.querySelector(".maptool_body").style.backgroundImage)
    }
    ...
    if (currentlyDeletingEffects) {
        ...
        for (var i = 0; i < effects.length; i++) {
            ...
            effects[i].onmousedown = function (event) {
                ...
                effects.splice(effects.indexOf(event.target), 1)
                if (pawns.lightSources.indexOf(event.target) >= 0) pawns.lightSources.splice(pawns.lightSources.indexOf(event.target), 1)
                window.requestAnimationFrame(refreshFogOfWar);
            }
        }
    } else {
    ...
    function measurementMouseDownHandler(event) {
        ...
        if (event.button == 0 && event.ctrlKey) {
            if (toolbox[2]) {
                ...
                console.log(div.dnd_height, div.dnd_width)
...
function generatePawns(pawnArray, monsters, optionalSpawnPoint) {
    ...
    for (var i = 0; i < pawnArray.length; i++) {
        ...
        console.log("Hexes " + newPawn.dnd_hexes)
        ...
        console.log(sightRadiusBright, sightRadiusDim, sightMode)
...
function raiseConditionsChanged(pawn) {
    console.log("Condtions changed", pawn["data-dnd_conditions"])
    let window2 = remote.getGlobal('mainWindow');
...
function enlargeReducePawn(direction) {
    console.log(creaturePossibleSizes)
...
function createBaseEffect(effectObj, isPreviewElement, e) {
    ...
    console.log(selectedSfxBackground)
    //Refresh preview
    if (!isPreviewElement)
...
function addPawnHandler(e) {
    ...
    console.log(pawnSizeDropDown.options[pawnSizeDropDown.selectedIndex].value)
...
function showPopupMenuAddEffect(event) {
    ...
    for (var i = 0; i < pawns.all.length; i++)pawns.all[i].classList.add("non_selectable");
...
function stopAddingEffects() {
    ...
    clearPreviewPlacement();
    gridLayer.style.cursor = "auto";
    for (var i = 0; i < pawns.all.length; i++)pawns.all[i].classList.remove("non_selectable");
    gridLayer.onmousedown = generalMousedowngridLayer;
...
function dragPawn(elmnt) {
    ...
    function elementDrag(e) {
        window.requestAnimationFrame(function (ts) {
            ...
            if (selectedPawns.length > 0) {
                for (var i = 0; i < selectedPawns.length; i++) {
                    selectedPawns[i].style.top = (selectedPawns[i].offsetTop - pos2) + "px";
                    selectedPawns[i].style.left = (selectedPawns[i].offsetLeft - pos1) + "px";
                }
...
function resizeEffects() {
    ...
    function resize(ele) {
        var width, height;
        width = parseInt(ele.dnd_width);
        height = parseInt(ele.dnd_height);
...
function resizeAndDrawGrid(timestamp, event) {
    ...
    saveTimer = window.setTimeout(
        function () {
            ...
            settings.gridSettings.mapSize = parseInt($("#background").css("background-size"));;
            saveSettings();
        }, 7000
    );
