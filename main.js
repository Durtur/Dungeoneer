...
const PathConstants = require("./main/pathConstants");
...
const GLOBAL_SETTINGS = Settings.globalSettingsSync();
const moduleLoader = new ModuleLoader(GLOBAL_SETTINGS.selectedModule);

Api.start(GLOBAL_SETTINGS.serverPort, [moduleLoader.commonModule(), moduleLoader.getModule()]);

function getWindow(windowName) {
    switch (windowName) {
        case "databaseWindow":
            return databaseWindow;
        case "generatorWindow":
            return generatorWindow;
        case "mainWindow":
            return mainWindow;
        case "maptoolWindow":
            return maptoolWindow;
        default:
            return null;
    }
}
...
const template = [
    ...
    {
        label: "View",
        submenu: [{ role: "forcereload" }, { role: "toggledevtools" }, { type: "separator" }, { type: "separator" }, { role: "togglefullscreen" }],
    },
    // { role: 'windowMenu' }
    {
        label: "Window",
...
if (!instanceLock) {
    ...
    app.on("second-instance", (event, commandLine, workingDirectory) => {
        ...
        if (mainWindow) {
            ...
            mainWindow.focus();
            if (commandLine) mainWindow.webContents.send("ipc-log", commandLine);
...
ipcMain.on("request-maptool-state", function () {
    if (maptoolWindow) maptoolWindow.webContents.send("get-state");
});

ipcMain.on("maptool-server-state", function (sender, arg) {
    if (maptoolWindow) maptoolWindow.webContents.send("maptool-server-state", arg);
});
...
ipcMain.on("config", function (event, options) {
    event.returnValue = {
        api: `${GLOBAL_SETTINGS.apiPath}:${GLOBAL_SETTINGS.serverPort}/`,
...
ipcMain.on("module-constants", function (event, options) {
    var pathToFile = PathConstants.moduleFile(moduleLoader.getModule().name, "constants.json");
    var json = Util.readJsonfile(pathToFile);
 
    event.returnValue = JSON.stringify(json);
});
...
function createWindow() {
    ...
    loading.once("show", () => {
        ...
        mainWindow = createBaseWindow(null, "app/index.html", null, false, false);
